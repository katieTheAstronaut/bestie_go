...
func setup(l int) (pubKey *pk, mk *BLS24479.ECP, alpha *BLS24479.BIG) {
	// ----------- Setup 1
	...
	q := BLS24479.NewBIGints(BLS24479.CURVE_Order) // TODO - maybe call this r
	...
	krands0 := make([]*BLS24479.BIG, l)    // slice for random numbers
	...
	krands1 := make([]*BLS24479.BIG, l)    // slice for random numbers
	...
	// omegatest := BLS24479.Ate(g2, mk)
	// omegatest = BLS24479.Fexp(omegatest)
	...
	// fmt.Println("Master Key (secret): ", mk, " - is this a point in G1? - ", BLS24479.G1member(mk), " ")
...
func keyGen(id string, mk *BLS24479.ECP, pubKey *pk, alpha *BLS24479.BIG) (secKey *sk, r *BLS24479.BIG, g1AlphaMinOmega, g1AlphaOmega *BLS24479.ECP) {

	// fmt.Println("\n\n")
	// fmt.Println("-------  KeyGen  ---------")
	// fmt.Println("The ID is: ", id, "")

	q := BLS24479.NewBIGints(BLS24479.CURVE_Order) // TODO - do not repeat yourself, consider putting this in main? but it needs to be secret
	l := len(id)
	// ----------- KeyGen 1
	...
	g1AlphaMinOmega = BLS24479.G1mul(pubKey.g1, exp) // g1^(alpha-alphaOmega)
	...
	// hExpTest := BLS24479.G1mul(x0Test, r)
	...
	xelements := make([]*BLS24479.ECP, l) // make a slice for x1 to xl
	...
	// y1,y3,...
	g1AlphaOmega = BLS24479.G1mul(pubKey.g1, alphaOmega) // g1^alphaOmega
	...
	for i := 0; i < l; i++ {
		if string(id[i]) == "0" { // TODO - perhaps make this more elegant and turn ID into slice instead of string? Then we wouldnt have to convert from string byte (string behaves like slice here) to string to compare both
			...
			yOdd[i] = temp
			temp2 := BLS24479.G1mul(pubKey.kelements0[i], r)
			yEven[i] = temp2
			...
			temp.Add(g1AlphaOmega)
			yOdd[i] = temp
			temp2 := BLS24479.G1mul(pubKey.kelements1[i], r)
			yEven[i] = temp2
			// fmt.Println(" y_", (i+1)*2-1, " : ", yOdd[i].ToString(), " ")
			// fmt.Println(" y_", (i+1)*2, " : ", yEven[i].ToString(), " ")
		} else {
...
func encrypt(cl, rl string, pubKey *pk, message *BLS24479.FP24, r *BLS24479.BIG, g1AlphaMinOmega *BLS24479.ECP) (cipher *hdr, krl *BLS24479.ECP) {
...
func decrypt(cl, rl, id string, secKey *sk, cipher *hdr, krl *BLS24479.ECP, r *BLS24479.BIG, g1AlphaOmega *BLS24479.ECP) (mes *BLS24479.FP24) {
