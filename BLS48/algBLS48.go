...
func setup(l int) (pubKey *pk, mk *BLS48581.ECP, alpha *BLS48581.BIG) {
	// ----------- Setup 1
	...
	q := BLS48581.NewBIGints(BLS48581.CURVE_Order) // TODO - maybe call this r
	...
	krands0 := make([]*BLS48581.BIG, l)    // slice for random numbers
	...
	krands1 := make([]*BLS48581.BIG, l)    // slice for random numbers
	...
	// omegatest := BLS48581.Ate(g2, mk)
	// omegatest = BLS48581.Fexp(omegatest)
...
func keyGen(id string, mk *BLS48581.ECP, pubKey *pk, alpha *BLS48581.BIG) (secKey *sk, r *BLS48581.BIG, g1AlphaMinOmega, g1AlphaOmega *BLS48581.ECP) {
	...
	g1AlphaMinOmega = BLS48581.G1mul(pubKey.g1, exp) // g1^(alpha-alphaOmega)
	...
	// hExpTest := BLS48581.G1mul(x0Test, r)
	...
	xelements := make([]*BLS48581.ECP, l) // make a slice for x1 to xl
	...
	// y1,y3,...
	g1AlphaOmega = BLS48581.G1mul(pubKey.g1, alphaOmega) // g1^alphaOmega
